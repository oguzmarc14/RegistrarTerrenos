...
const $$Terrenos = createComponent(async ($$result, $$props, $$slots) => {
  const { data: terrenos, error } = await supabase.from("Terrenos").select("id, titulo, descripcion, precio, imagenes, medidas, ubicacion, tipo, fecha").eq("tipo", "terreno").order("fecha", { ascending: false });
...
Terrenos en venta y renta en Encarnación de Díaz
</h2> <p class="text-sky-100/80 max-w-4xl mx-auto mt-4">
Encuentra terrenos ideales para inversión, construcción o desarrollo comercial. Consulta precios, ubicación y características actualizadas.
...
Aquí puedes ver todos los terrenos actualmente en venta o renta.
</p> </div> ${error && renderTemplate`<p class="text-red-500 text-center">Error al cargar terrenos</p>`} <div class="grid gap-10 sm:gap-12 md:gap-14 grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 mx-auto px-4 md:px-8 mb-12"> ${Array.isArray(terrenos) && terrenos.map((terreno) => renderTemplate`${renderComponent($$result2, "CardMostrarTerreno", $$CardMostrarTerreno, { "id": terreno.id, "imagenes": terreno.imagenes, "titulo": terreno.titulo, "descripcion": terreno.descripcion, "precio": terreno.precio, "textBoton": "Ver m\xE1s", "hrefBoton": "#", "medidas": terreno.medidas, "ubicacion": terreno.ubicacion, "fecha": terreno.fecha, "tipo": terreno.tipo })}`)} </div> ${renderComponent($$result2, "Footer", $$Footer, {})} </section> ` })}`;
}, "C:/Users/Oguzm/OneDrive/Documentos/GitHub/RegistrarTerrenos/src/pages/TipoTerreno/Terrenos.astro", void 0);

const $$file = "C:/Users/Oguzm/OneDrive/Documentos/GitHub/RegistrarTerrenos/src/pages/TipoTerreno/Terrenos.astro";
const $$url = "/TipoTerreno/Terrenos";
